...
async def cookie2user(cookie_str):
    ...
    try:
        ...
        if len(L) != 3:           # 由上可知，cookie有３部分组成，若拆分得到不是３部分，显然出错了
            return None
        uid, expires, sha1 = L
        if int(expires) < time.time():  # 时间是浮点表示时间戳，一直再增大，因此失效时间小于但前时间，说明cookie已失效
            return None
...
# 这是实现用户注册的api,注册到/api/users路径上，http method 为post
@post('/api/users')
def api_register_user(*, name, email, passwd):     # 注册信息包括用户名，邮箱域密码
    # 验证输入的正确性
    ...
    if len(users) > 0:     # findAll的结果不为０，说明数据库已存在同名emial, 抛出异常报错
        raise APIError('register:failed', 'email', 'Email is already in use.')

    # 数据库内无相应的email信息，说明是第一次注册
    uid = next_id()     # 利用当前时间域随机生成的uuid生成user id
    sha1_passwd = '%s:%s' % (uid, passwd)     # 将user id 与密码的组合赋给sha1_passwd变量
    # 创建用户对象其中密码并不是用户输入的密码，而是经过复杂处理后的保密字符串
    # unicode 对象进行哈溪运算之前必须先编码
    # sha1(secure hash algorithm), 是一种不可逆的安全算法，这在一定程度上保证了安全性，因为用户密码只有用户一人知道
    # md5是另一种安全算法
    # Gravatar(Globally Recognized Avatar)是一项用于提供在全球范围内使用的头像服务。只要在Gravatar的服务器上上传了你自己的头像，便可在其它任何支持Gravatar的博客，论坛等地方使用它。此处image就是一个根据用户email生成的头像
    ...
    r = web.Response()
    # 刚建立的用户设置cookie(网站为了辨别用户身份而储存在用户本地终端的数据)
    # http协议是一种无状态的协议，即服务器并不知道用户上一次做了什么。
    # 因此服务器可以通过设置或读取Cookies中包含信息，借此维护用户跟服务器会话中的　状态
    # user2cookie设置的是cookie的值
    # max_age是cookie的最大存活中期，单位是秒，当时间结束时，客户端将抛弃该cookie. 之后需要重新登录
...
def authenticate(*, email, passwd):
    ...
    # 验证密码
    # 数据路中存储的并非原始的用户密码，而是加密的字符串
    # 将用户输入的密码做相同的加密处理，再和数据库中的密码进行比对，来验证密码的真实性
    # 以下步骤合成一步就是：sha1 = hashlib.sha1((user.id+":"+passwd).encode("utf-8"))
    # 对照用户时对原始密码的操作见：api_register_user, 操作完全一样
    sha1 = hashlib.sha1()
