...
def init_jinja2(app, **kw):
    ...
    options = dict(
        ...
        auto_reload = kw.get("auto_reload", True)                      # 每当对模板发起请求，加载器首先检查模板是否发生改变，若是，重新加载模板
    )
    ...
    # 加载器负责从指定位置加载模板，此处选择FileSystemLoader,顾名思义就是文件系统加载模板，前面已经设置了path
    env = Environment(loader = FileSystemLoader(path), **options)
    # 设置过滤器
    # 先通过filters关键字参数获取过滤字典
    # 在通过建立env.filter的键值对建立过滤器
...
# 创建应用是，通过指定命名关键字为一些"middle factory"的列表可创建中间键Middleware
# 每个middle factory收到２个参数，一个app实例，一个handler, 并返回一个新的handler
...
async def data_factory(app, handler):
    async def parse_data(request):
        # 解析数据是针对post方法传来的数据，若http method 非 post,将跳过，直接调用handler处理请求
        if request.method == "POST":
            ...
            # content type字段以application/x-www-form-urlencoded打头的，是浏览器表单
...
# 上面２个middle factory是在url处理函数之前先对请求进行了处理，以下则在url处理函数之后进行处理
# 其将request handler的返回值转换为web.Response对象
async def response_factory(app, handler):
    async def response(request):
        logging.info("Response handler...")
        # 调用handler来处理url请求，并返回相应结果
        r = await handler(request)
        # 响应结果为StreamResponse, 直接返回
        ...
        if isinstance(r, str):
            ...
            if r.startswith("redirect:"):
                return web.HTTPFoud(r[9:])
            # 响应结果不是重定向，则以utf-8对字符串进行编码，作为body,设置相应的响应类型
            resp = web.Response(body = r.encode("utf-8"))
            resp.content_type = "text/html;charset=utf-8"
            return resp
        # 若响应结果为字典，则获取他的模板属性，此处为jinja2.env(见init_jinja2)
